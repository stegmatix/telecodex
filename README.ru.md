...
- Опциональная транскрибация через Groq Whisper.
- Если задан `GROQ_API_KEY`, Telecodex расшифровывает audio и voice-вложения перед отправкой хода в Codex.
...
Если хочешь запускать Telecodex у себя и держать собственные изменения, самый чистый вариант такой:

1. Сделать fork upstream-репозитория на GitHub.
2. Держать remotes в стандартном виде:
   - `origin` → твой fork
   - `upstream` → `Headcrab/telecodex`
3. Коммитить только переиспользуемые изменения в коде и документации.
4. Не хранить в git локальные runtime-файлы:
   - `telecodex.toml`
   - `.env`
   - `telecodex.sqlite3`
   - `telecodex.log`
...
Для своих доработок лучше использовать короткие feature-ветки.
...
Если собираешься держать свои патчи, клонируй fork, а не upstream.
...
Перед запуском задай `TELECODEX_TELEGRAM_BOT_TOKEN` в окружении. Пример:
...
Для постоянного локального запуска лучше использовать `tmux` или `systemd --user`.
...
- `TELECODEX_TELEGRAM_BOT_TOKEN`: токен Telegram Bot API.
- `GROQ_API_KEY`: включает транскрибацию аудио и voice-сообщений через Groq Whisper.
- `GROQ_TRANSCRIPTION_MODEL`: необязательное переопределение модели Groq, по умолчанию `whisper-large-v3-turbo`.
